...
const ProjectList = ({projectCollection = []}) => {
  const [list, setList] = useState(projectCollection)
  ...
  const debounced = useRef(
    debounce(() => {
      ...
      gridRef.current.style.height = `${columnHeight / columnCount + winWidth * 0.4}px`
    }, 200)
  )
  ...
  useEffect(() => {
    if (!client) {
      setList(projectCollection)
    } else {
      const filter = client?.toLowerCase()
      const filteredList = projectCollection.filter(
        (item) => get(item, 'client', '').toLowerCase() === filter
      )
      setList(filteredList)
    }
  }, [projectCollection, client])
  ...
  return (
    <section className={css.wrap}>
      <div className={css.masonry} ref={gridRef} data-test-id="masonry-wrap">
        {list.map((project) => (
          <div className={css.item} key={project._id}>
            <ProjectItem {...project} />
          </div>
        ))}
      </div>
